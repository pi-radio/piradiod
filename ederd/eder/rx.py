...
from .registers import attach_registers, set_bits, clear_bits
from .child import EderChild
...
class RX(Beamformer):
    ...
    def run_drv_cal(self):
        ...
        print(f"DRV CAL: {self.I.drv_offset} {self.Q.drv_offset}")
        self.config["rx_I_drv_offset"] = self.I.drv_offset
        self.config["rx_Q_drv_offset"] = self.Q.drv_offset
    ...
    def run_dco_cal(self):
        ...
        print(f"DCO CAL: {self.I.dco_reg} {self.Q.dco_reg}")
        self.config["rx_I_dco_reg"] = self.I.dco_reg
        self.config["rx_Q_dco_reg"] = self.Q.dco_reg
    ...
    def startup(self):
        ...
        if "rx_I_drv_offset" in self.config and "rx_Q_drv_offset" in self.config:
            self.I.drv_offset = self.config["rx_I_drv_offset"]
            self.Q.drv_offset = self.config["rx_Q_drv_offset"]
        else:
            print(f"Running RX DRV cal: {self.config._config}")
            self.run_drv_cal()

        if "rx_I_dco_reg" in self.config and "rx_Q_dco_reg" in self.config:
            self.I.dco_reg = self.config["rx_I_dco_reg"]
            self.Q.dco_reg = self.config["rx_Q_dco_reg"]
        else:
            print(f"Running RX DCO cal: {self.config._config}")
            self.run_dco_cal()
